        ...
        <table style="width:100%">
            ...
            <tr> 
                ...
                <?php 
                    ...
                    // get 0-10 range for similarity threshold from post request
                    $range = $_POST['slider'];
                    
                    // choose range so that 0->1 limit to 10->50001 limit
                    // 0 slider --> accepts only exact same pixels
                    // 10 slider --> accepts everything
                    $limit = 5000 * $range + 1;
                    ...
                    foreach($hex_arr as $hex)
                    {
                        ...
                        $rgb_vals = array(0=>$r_val, 1=> $g_val, 2=>$b_val);
                    ...
                    print("<img src=../{$name} max-width='500px'>");
                    ...
                    for($x=0; $x<$width; $x++)
                    {
                        ...
                        for($y=0; $y<$height; $y++)
                        {
                            ...
                            //If the shortest distance is lower than the limit,
                            //Keep the original pixel's RGB value for the new image
                            if($min < $limit)
                            {
                                $col = imagecolorallocate($im, $oldr[$x][$y], $oldg[$x][$y], $oldb[$x][$y]);
                                imagesetpixel ($im , $x , $y, $col);    
                            }
                            //If the shortest distance is larger than the limit,
                            //Switch the pixel to grayscale by averaging RGB values
                            else 
                            {
                    ...
                    imagepng($im, "../img/colorpop/{$name}");
                    
                    //Display the new image in the right pannel 
                    print("</td>
                    <td>
                        <img src=../img/colorpop/{$name} max-width='500px'>
                    </td>")
                    ?>
